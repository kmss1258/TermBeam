...
const GRADIENT_BG = 'radial-gradient(ellipse at 40% 50%, #1a1a3e 0%, #0f0c29 50%, #0a0a1a 100%)';
...
const MobileTermScreen: React.FC<{ frame: number; fps: number }> = ({ frame }) => {
  ...
  const cmd2Chars = Math.min(Math.max(0, Math.floor(cmd2Frame / TYPE_SPEED)), CMD2.length);
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          />
          <span style={{ color: '#e6edf3', fontWeight: 700, fontSize: 13, fontFamily: monoFont }}>
            pty-mirror
          </span>
        </div>
        <span style={{ color: '#8b949e', fontSize: 11, fontWeight: 500 }}>Connected</span>
      </div>
      ...
      >
        ...
        <div style={{ display: 'flex' }}>
          ...
          {!cmd1Done && <span style={{ color: '#a78bfa', opacity: cursorBlink ? 1 : 0 }}>▋</span>}
        </div>
...
export const MobileTerminal: React.FC = () => {
  ...
  const zoomIn = interpolate(frame, [ZOOM_IN_START, ZOOM_IN_END], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  // Ease out cubic
  ...
  const PAN_START = 160;
  const PAN_END = 200;
  const panProgress = interpolate(frame, [PAN_START, PAN_END], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  ...
  const zoomOutProgress = interpolate(frame, [ZOOM_OUT_START, ZOOM_OUT_END], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  ...
  const titleFade = interpolate(frame, [ZOOM_IN_START, ZOOM_IN_START + 20], [1, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
